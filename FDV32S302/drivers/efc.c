...
/**
  ...
  *     @arg EFC_OPR_MODE_BLOCK_ERASE: Row Program mode
...
/**
  * @brief  EFC erases the specified block.
  * @param  Addr: Specify the address to Block Erase.
  * @retval EFC processing result, the return value can be one of @ref EFC_Status enumeration.
  */
EFC_Status EFC_BlockErase(u32 Addr)
{
	...
	/* Check the parameters */
	PARAM_CHECK(IS_EFC_EEPROM_ADDR(Addr));

	/* Clear EFC Status Register */
	tempreg	 = EFC->STS;
	EFC->STS = tempreg;

	/* The OPR register of the EFC performs the block erase operation */
