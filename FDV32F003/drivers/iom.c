...
/**
  * @brief  Set the JTAG usage of a multiplexed port.
  * @param  JtagUse: Specifies the JTAG port usage.
...
void IOM_SetJtagUse(u8 JtagUse)
{
	...
	/* Check the parameters */
	PARAM_CHECK(IS_IOM_JTAGUSE(JtagUse));

	/* Get the IOM_CTL value */
	tempreg = IOM->CTL;

	if(JtagUse == IOM_JTAG_USE_JTAG)
	{
		/* Set JTAG port to act as JTAG */
		tempreg |= IOM_CTL_JTAG_USE_IO;
	}
	else
	{
		/* Set JTAG port to act as GPIO */
		tempreg &= ~IOM_CTL_JTAG_USE_IO;
	}
...
/**
  ...
  *   This parameter can be any combination of GPIO_PINx where x can be (0..16).
...
/**
  ...
  *   This parameter can be GPIO_PINx where x can be (0..16).
