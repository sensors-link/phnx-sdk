...
/**
  * @brief  Set the low power clock of the normally-on power domain.
  * @param  LPClock: Specify the normally-on power domain Low Power clock.
  *   This parameter can be one of the following values:
  *     @arg PMU_LP_CLOCK_LRC: Normally open power domain Low Power clock select LRC.
  *     @arg PMU_LP_CLOCK_XTL: Normally open power domain Low Power clock select XTL.
  * @retval None
  */
void PMU_SetLowPowerClock(u8 LPClock)
{
	/* Check the parameters */
	PARAM_CHECK(IS_PMU_LP_CLOCK(LPClock));

	/* Set the normally-on power domain Low Power clock */
	if (LPClock == PMU_LP_CLOCK_LRC)
	{
		/* Set the normally-on power domain Low Power clock select LRC */
		PMU_WPT_UNLOCK();
		PMU->CR &= ~PMU_CR_LPTCLKSEL;
	}
	else
	{
		/* Set the normally-on power domain Low Power clock select XTL */
		PMU_WPT_UNLOCK();
		PMU->CR |= PMU_CR_LPTCLKSEL;
	}
}
...
/**
  ...
  *     @arg PMU_FLAG_LPUWK: LPUart wake-up status flag
  *     @arg PMU_FLAG_RTCWK: RTC wake-up status flag
