...
#define  EFC_OPR_OPEN(mode)        {EFC_OPR_REG = EFC_OPR_PPRF_V0 | (mode); EFC_OPR_REG = EFC_OPR_PPRF_V1 | (mode); EFC_OPR_REG = EFC_OPR_PPRF_V2 | (mode); EFC_OPR_REG = EFC_OPR_PPRF_V3 | (mode);}
...
{
    PARAM_CHECK( (Addr<0x10100000) || ( (Addr>0x1011ffff)&&(Addr<0x10140000) ) || ((Addr>0x10140dff) && (Addr<0x10180000) ) || (Addr>0x101803ff));
 ...
 * @note :u32Addr page锛�512Byte锛塧lign, only flash and nvr1-7 region valid
 */
 ...
 * @note:eeprom only erase one word锛�4Byte锛�
 */
eReturnType EFC_PageErase(u32 u32Addr)
{
    PARAM_CHECK( (u32Addr<0x10100000) || ( (u32Addr>0x1011ffff)&&(u32Addr<0x10140000) ) || ( (u32Addr>0x10140dff)&& (u32Addr<0x10180000)) || (u32Addr>0x101803ff));
    u32 stat = EFC_STS_REG;
    EFC_STS_REG = stat;
 ...
 * @param u32PrgType 锛欵FC_FLASH_WRITE_WORD , EFC_FLASH_WRITE_PAGE
 * @return eReturnType
