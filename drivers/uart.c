 ...
 * @param port:UART1_PORT_P00_P01,UART1_PORT_P16_P17,UART2_PORT_P02_P03,UART2_PORT_P04_P05,UART2_PORT_P14_P15,LPUART_PORT_P18_P19
...
void UART_Init(UART_Type *psUart,int port,int mode,int iBaud)
{
    ...
    if( psUart == UART1)
    {
        ...
        {
            PARAM_CHECK( pclk/2/iBaud < 2);
            psUart->BDIV = (pclk+pclk/4/iBaud)/2/iBaud;
        ...
        else if(mode == UART_MODE_10B_ASYNC)
        {
            ...
            psUart->BDIV = (pclk+pclk/32/iBaud)/16/iBaud;
    ...
    else if( psUart == LPUART)
    {
        ...
        if(mode == UART_MODE_8B_SYNC)
        {
            PARAM_CHECK( 600000/2/iBaud < 2);
            psUart->BDIV = (600000+600000/4/iBaud)/2/iBaud;
            psUart->SCON = ( (UART_MODE_8B_SYNC)<<UART_SCON_SM01_pos) | UART_SCON_REN;
        }
        else if(mode == UART_MODE_10B_ASYNC)
        {
            PARAM_CHECK( 600000/16/iBaud < 1);
            psUart->BDIV = (600000+600000/32/iBaud)/16/iBaud;
            psUart->SCON = ( (UART_MODE_10B_ASYNC)<<UART_SCON_SM01_pos) | UART_SCON_REN;
        }
        else
        {
 ...
 *
 * @param psUart :UART1,UART2,LPUART
 * @param enType :UART_EN_TYPE_LPMODE | UART_EN_TYPE_SM2 | UART_EN_TYPE_REN | UART_EN_TYPE_SFD
 */
