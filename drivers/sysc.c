 ...
 *
 * @param clkSrc :SYSC_CLK_SRC_HRC  , SYSC_CLK_SRC_LRC , SYSC_CLK_SRC_XTH , SYSC_CLK_SRC_XTL
 */
void SYSC_SelectClockSource(int clkSrc)
{
    PARAM_CHECK( (clkSrc != SYSC_CLK_SRC_HRC) && (clkSrc != SYSC_CLK_SRC_LRC) && (clkSrc != SYSC_CLK_SRC_XTH) && (clkSrc != SYSC_CLK_SRC_XTL));
    SYSC_WPT_UNLOCK();
