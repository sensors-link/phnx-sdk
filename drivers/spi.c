...
void SPI_Init(int pin, int mode, int pol, int phase, int freq) {
    ...
    PARAM_CHECK((pin != SPI_PIN_0_1_2_3) && (pin != SPI_PIN_12_13_14_15) &&
                (pin != SPI_PIN_19_18_16_17));
    if (pin == SPI_PIN_0_1_2_3) {
        IOM->AF0 &= ~(IOM_AF0_P00_SEL | IOM_AF0_P01_SEL | IOM_AF0_P02_SEL |
                      IOM_AF0_P03_SEL);
        IOM->AF0 |= (IOM_AF0_P00_SEL_SPI_CS | IOM_AF0_P01_SEL_SPI_SCK |
                     IOM_AF0_P02_SEL_SPI_MOSI | IOM_AF0_P03_SEL_SPI_MISO);
    } else if (pin == SPI_PIN_12_13_14_15) {
        IOM->AF0 &= ~(IOM_AF0_P12_SEL | IOM_AF0_P13_SEL | IOM_AF0_P14_SEL |
                      IOM_AF0_P15_SEL);
        IOM->AF0 |= (IOM_AF0_P12_SEL_SPI_CS | IOM_AF0_P13_SEL_SPI_SCK |
                     IOM_AF0_P14_SEL_SPI_MOSI | IOM_AF0_P15_SEL_SPI_MISO);
    } else {
        IOM->AF1 &= ~(IOM_AF1_P19_SEL | IOM_AF1_P18_SEL | IOM_AF1_P16_SEL |
                      IOM_AF1_P17_SEL);
        IOM->AF1 |= (IOM_AF1_P19_SEL_SPI_CS | IOM_AF1_P18_SEL_SPI_SCK |
                     IOM_AF1_P16_SEL_SPI_MOSI | IOM_AF1_P17_SEL_SPI_MISO);
    }
    ...
    if (mode == SPI_MASTER) {
        ...
        PARAM_CHECK((SystemCoreClock / (freq * 1000) < 2) ||
                    (SystemCoreClock / (freq * 1000) > 256));
 ...
 *
 */
void SPI_EnableIRQ(void) { SPI->CR0 |= SPI_CR0_DONEIE; }
/**
 * @brief trasfer done interrupt disable
 *
 */
void SPI_DisableIRQ(void) { SPI->CR0 &= ~SPI_CR0_DONEIE; }

/**
 * @brief set CS high
 *
 */
void SPI_SetCSN(void) { SPI->CSN = 0x01; }
/**
 * @brief set cs low
 *
 */
void SPI_ClrCSN(void) { SPI->CSN = 0; }
 ...
 */
void SPI_SendData(u8 data) { SPI->DR = data; }
/**
 * @brief recieve data
 *
 * @return u8 :8bit data
 */
u8 SPI_RecieveData(void) { return SPI->DR; }
 ...
 */
u32 SPI_GetStatus(void) { return SPI->SR; }
